...
<p align="center">
  极简无广告浏览器新标签页 · 内置 AI 对话<br>
  <em>A minimal, ad-free browser new tab page with built-in AI chat</em>
</p>
<p align="center">
...
**Quick Dial（呲啦起始页）** 替换 Chrome/Edge 的默认新标签页，内置快捷导航、多引擎搜索、AI 对话、天气、农历、RSS、精美壁纸。所有数据存储在本地浏览器，**无广告、无追踪、不收集任何信息**。

**Quick Dial** replaces Chrome/Edge's default new tab with quick dials, multi-engine search, AI chat, weather, lunar calendar, RSS, and beautiful wallpapers. All data stays in your browser — **no ads, no tracking, no data collection**.
...
*This project (Quick Dial) is registered with the National Copyright Administration of China — Software Copyright Registration No. `2026SR0795653`. Unauthorized copying, distribution, or modification is prohibited.*
...
| | 中文 | English |
|---|------|---------|
| 🔍 | **多引擎搜索** — 13 种内置引擎（Google / 百度 / Bing / 知乎 / B站 / GitHub / DuckDuckGo / X / YouTube 等），Ctrl+K 聚焦 | **Multi-Engine Search** — 13 built-in engines (Google, Baidu, Bing, Zhihu, Bilibili, GitHub, DuckDuckGo, X, YouTube, etc.). Ctrl+K to focus |
| 🤖 | **AI 对话** — 14 家 AI 提供商（DeepSeek / OpenAI / 通义千问 / Kimi / 智谱 / 文心 / Claude / Mistral / Groq / 混元 / 豆包 / 硅基流动 / Together / Ollama），悬浮对话框 | **AI Chat** — 14 AI providers (DeepSeek, OpenAI, Qwen, Kimi, GLM, ERNIE, Claude, Mistral, Groq, Hunyuan, Doubao, SiliconFlow, Together, Ollama). Floating dialog |
| 📌 | **快捷导航** — 拖拽排序、分组管理、70+ 图标库，粘贴网址自动识别 | **Quick Dials** — Drag & drop, groups, 70+ icon library. Paste URL to auto-detect |
| 🖥️ | **飞牛NAS** — 已上架飞牛应用商店，一键安装，自动发现 NAS 应用 | **fnOS App** — Available on fnOS App Store, one-click install, auto-discover NAS apps |
| 🎨 | **精美壁纸** — 12 种预设渐变（深海、极光、日落、樱花……），自适应深浅文字 | **Beautiful Wallpapers** — 12 preset gradients. Auto-adapts light/dark text |
| 🌤️ | **天气与农历** — 实时天气、温度、节气、黄历日期，移动端胶囊样式 | **Weather & Lunar** — Live weather, solar terms, lunar dates. Mobile capsule style |
| 🕐 | **多款时钟** — 6 种样式（数字/极简/经典/翻页/霓虹/二进制） | **Clock Widgets** — 6 styles (Digital, Minimal, Classic, Flip, Neon, Binary) |
| 📰 | **RSS 订阅** — 浮动对话框浏览订阅源 | **RSS Reader** — Floating dialog to browse feeds |
| 💬 | **一言 / Quotes** — Hitokoto 随机语录或自定义格言 | **Quotes** — Hitokoto random quotes or custom motto |
...
|------|:---:|:---:|
| 搜索引擎 Search Engines | 5 + 淘宝 | 全部 13 种 + 自定义 / All 13 + custom |
| 导航分组 Dial Groups | 3 | 无限 Unlimited |
| AI 对话 AI Chat | 1 引擎 (DeepSeek) | 全部 14 家 / All 14 providers |
| AI 对话历史 Chat History | — | ✓ |
| AI 系统提示词 System Prompt | — | ✓ |
| 自定义壁纸 Custom Wallpaper | — | ✓ |
| 壁纸定时切换 Wallpaper Auto-Switch | — | ✓ |
| 自定义 CSS Custom CSS | — | ✓ |
| 数据看板 Data Dashboard | — | ✓ |
| 云端同步 Cloud Sync | — | ✓ |
| 价格 Price | ¥0 终身 Lifetime | ¥9.9/月 · ¥68/年 · ¥198终身 / $1.49/mo · $9.49/yr · $27.49 lifetime |
...
| [FontAwesome 7](https://fontawesome.com/) | 图标库 / Icon library |
...
│   ├── utils/
│   │   ├── ai.ts                    # AI 对话（14 家提供商） / AI chat (14 providers)
...
│   │   ├── fnos.ts                  # 飞牛应用自动发现
│   │   ├── contextMenu.ts           # 右键菜单
│   │   ├── modalClose.ts            # 模态框关闭逻辑
│   │   ├── i18n.svelte.ts           # 多语言
│   │   └── toast.svelte.ts          # Toast 通知
│   ├── stores/
│   │   ├── chat.svelte.ts           # AI 对话状态
...
│   │   ├── rss.svelte.ts            # RSS 状态
│   │   ├── quote.svelte.ts          # 一言/格言
│   │   ├── horoscope.svelte.ts      # 星座（待清理） / Horoscope (legacy)
│   │   └── pomodoro.svelte.ts       # 番茄钟（待清理） / Pomodoro (legacy)
│   └── components/
│       ├── SearchBox.svelte         # 搜索框
│       ├── AIWidget.svelte          # AI 悬浮对话框 / AI floating chat
...
│       ├── QuoteWidget.svelte       # 一言/格言
│       ├── RssWidget.svelte         # RSS 订阅
