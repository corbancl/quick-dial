...
// 自动迁移：速率限制（防盗刷）
try { $db->exec("CREATE TABLE IF NOT EXISTS `qd_rate_limit` (
    `key` VARCHAR(64) NOT NULL,
    `hits` INT NOT NULL DEFAULT 0,
    `window_start` INT NOT NULL DEFAULT 0,
    `block_until` INT NOT NULL DEFAULT 0,
    PRIMARY KEY (`key`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"); } catch (Exception $e) {}

// 自动迁移：性能监控日志
try { $db->exec("CREATE TABLE IF NOT EXISTS `qd_perf_log` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
...
// 自动迁移：操作通知
try { $db->exec("CREATE TABLE IF NOT EXISTS `qd_notifications` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
...
// 自动迁移：多管理员（角色 + 上级）
try { $db->exec("ALTER TABLE qd_admins ADD COLUMN role VARCHAR(20) NOT NULL DEFAULT 'super'"); } catch (Exception $e) {}
try { $db->exec("ALTER TABLE qd_admins ADD COLUMN parent_id INT NULL DEFAULT NULL"); } catch (Exception $e) {}
try { $db->exec("UPDATE qd_admins SET role = 'super' WHERE role = '' OR role IS NULL"); } catch (Exception $e) {}
...
// ====== 新增能力：速率限制 / 性能日志 / 操作通知 / 多管理员 ======

// 速率限制：每 $window 秒最多 $limit 次，超限封禁 $block 秒。返回 true=放行
function rateLimit(PDO $db, string $key, int $limit, int $window, int $block = 900): bool {
    $now = time();
    $row = q1($db, 'SELECT * FROM qd_rate_limit WHERE `key` = ?', [$key]);
    if ($row && (int)$row['block_until'] > $now) return false;
    if (!$row || (int)$row['window_start'] + $window <= $now) {
        $db->prepare('REPLACE INTO qd_rate_limit (`key`, hits, window_start, block_until) VALUES (?, 1, ?, 0)')
           ->execute([$key, $now]);
        return true;
    }
    $hits = (int)$row['hits'] + 1;
    if ($hits > $limit) {
        $db->prepare('UPDATE qd_rate_limit SET block_until = ? WHERE `key` = ?')->execute([$now + $block, $key]);
        return false;
    }
    $db->prepare('UPDATE qd_rate_limit SET hits = ? WHERE `key` = ?')->execute([$hits, $key]);
    return true;
}

// 性能日志
function logPerf(PDO $db, string $endpoint, float $ms): void {
    try { $db->prepare('INSERT INTO qd_perf_log (endpoint, ms) VALUES (?, ?)')->execute([$endpoint, (int)round($ms)]); } catch (Exception $e) {}
}

// 操作通知（站内）
function notify(PDO $db, string $type, string $title, string $detail = ''): void {
    try { $db->prepare('INSERT INTO qd_notifications (type, title, detail) VALUES (?, ?, ?)')->execute([$type, $title, $detail]); } catch (Exception $e) {}
}

// 标记通知已读
function markNotifyRead(PDO $db, int $id): void {
    try { $db->prepare('UPDATE qd_notifications SET is_read = 1 WHERE id = ?')->execute([$id]); } catch (Exception $e) {}
}

// 性能监控：记录每次请求耗时（shutdown 时写入，覆盖所有 exit 路径）
$qd_t0 = microtime(true);
register_shutdown_function(function () use ($db, $qd_t0) {
    try {
        $ms = (microtime(true) - $qd_t0) * 1000;
        $path = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH);
        $ep = ($_SERVER['REQUEST_METHOD'] ?? 'GET') . ' ' . ($path ?: '/');
        $db->prepare('INSERT INTO qd_perf_log (endpoint, ms) VALUES (?, ?)')->execute([$ep, (int)round($ms)]);
    } catch (Exception $e) {}
});
...
if (!$loggedIn && isset($_POST['username']) && isset($_POST['password'])) {
    $clientIp = $_SERVER['HTTP_X_FORWARDED_FOR'] ?? ($_SERVER['HTTP_CLIENT_IP'] ?? ($_SERVER['REMOTE_ADDR'] ?? '0'));
    if (!rateLimit($db, 'login:' . $clientIp, 10, 300, 900)) {
    ...
    if ($lockUntil > time()) {
        ...
        if ($captchaOk) {
            ...
            if ($admin && password_verify($_POST['password'], $admin['password'])) {
                ...
                $_SESSION['qd_admin_role'] = $admin['role'] ?? 'super';
...
// ====== 数据备份导出（需登录）======
if ($loggedIn && ($_GET['do'] ?? '') === 'backup_export') {
    $tables = ['qd_users', 'qd_subscriptions', 'qd_orders', 'qd_sync_log', 'qd_admin_log', 'qd_admins', 'qd_notifications', 'qd_perf_log', 'qd_rate_limit'];
    $data = [];
    foreach ($tables as $t) {
        try { $data[$t] = q($db, "SELECT * FROM `$t`"); } catch (Exception $e) { $data[$t] = ['__error' => $e->getMessage()]; }
    }
    $data['__exported_at'] = date('Y-m-d H:i:s');
    $json = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
    header('Content-Type: application/json; charset=utf-8');
    header('Content-Disposition: attachment; filename="quickdial-backup-' . date('YmdHis') . '.json"');
    echo $json;
    exit;
}
...
if ($loggedIn && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (($input = json_decode(file_get_contents('php://input'), true)) || $_POST['action'] ?? '') {
        ...
        // 通知：标记已读（单条或全部）
        if ($act === 'mark_notify') {
            $nid = intval($input['id'] ?? $_POST['id'] ?? 0);
            if ($nid) { markNotifyRead($db, $nid); } else { $db->exec('UPDATE qd_notifications SET is_read = 1'); }
            echo json_encode(['code' => 200]); exit;
        }

        // 管理员管理（仅超管）
        $isSuper = ($_SESSION['qd_admin_role'] ?? 'super') === 'super';
        if ($act === 'add_admin') {
            if (!$isSuper) { echo json_encode(['code' => 403, 'msg' => '仅超管可操作']); exit; }
            $uname = trim($input['username'] ?? $_POST['username'] ?? '');
            $upass = $input['password'] ?? $_POST['password'] ?? '';
            $urole = $input['role'] ?? $_POST['role'] ?? 'staff';
            if (!$uname || strlen($upass) < 6) { echo json_encode(['code' => 400, 'msg' => '账号或密码(≥6位)无效']); exit; }
            if (q1($db, 'SELECT id FROM qd_admins WHERE username = ?', [$uname])) { echo json_encode(['code' => 400, 'msg' => '账号已存在']); exit; }
            $db->prepare('INSERT INTO qd_admins (username, password, role, parent_id) VALUES (?, ?, ?, ?)')
               ->execute([$uname, password_hash($upass, PASSWORD_DEFAULT), $urole, $_SESSION['qd_admin_id'] ?? null]);
            logAdmin($db, 'add_admin', "新增管理员 {$uname} ({$urole})");
            notify($db, 'security', '新增管理员', "{$uname} ({$urole})");
            echo json_encode(['code' => 200, 'msg' => '已添加']); exit;
        }
        if ($act === 'del_admin') {
            if (!$isSuper) { echo json_encode(['code' => 403, 'msg' => '仅超管可操作']); exit; }
            $aid = intval($input['id'] ?? $_POST['id'] ?? 0);
            if ($aid == ($_SESSION['qd_admin_id'] ?? 0)) { echo json_encode(['code' => 400, 'msg' => '不能删除自己']); exit; }
            $db->prepare('DELETE FROM qd_admins WHERE id = ? AND role != ?')->execute([$aid, 'super']);
            logAdmin($db, 'del_admin', "删除管理员#{$aid}");
            echo json_encode(['code' => 200, 'msg' => '已删除']); exit;
        }
        if ($act === 'set_admin_role') {
            if (!$isSuper) { echo json_encode(['code' => 403, 'msg' => '仅超管可操作']); exit; }
            $aid = intval($input['id'] ?? $_POST['id'] ?? 0);
            $urole = $input['role'] ?? $_POST['role'] ?? 'staff';
            $db->prepare('UPDATE qd_admins SET role = ? WHERE id = ? AND role != ?')->execute([$urole, $aid, 'super']);
            logAdmin($db, 'set_admin_role', "管理员#{$aid} → {$urole}");
            echo json_encode(['code' => 200, 'msg' => '已更新']); exit;
        }
        ...
        if ($act === 'activate') {
            ...
            notify($db, 'order', 'Pro 开通/续费', "用户#{$uid} {$plan}" . ($note ? " 备注:{$note}" : ''));
        ...
        if ($act === 'ban') {
            $uid = intval($input['user_id'] ?? $_POST['user_id'] ?? 0);
            if ($uid) { $db->prepare('UPDATE qd_users SET banned = 1 WHERE id = ?')->execute([$uid]); $msg = "用户 #{$uid} 已封禁"; logAdmin($db, 'ban', "用户#{$uid}"); notify($db, 'security', '用户封禁', "用户#{$uid}"); }
        }
        if ($act === 'unban') {
            $uid = intval($input['user_id'] ?? $_POST['user_id'] ?? 0);
        ...
        if ($act === 'cancel_pro') {
            $uid = intval($input['user_id'] ?? $_POST['user_id'] ?? 0);
            if ($uid) { $db->prepare('UPDATE qd_subscriptions SET plan = "free", expire_at = NOW() WHERE user_id = ?')->execute([$uid]); $msg = "用户 #{$uid} Pro 已取消"; logAdmin($db, 'cancel_pro', "用户#{$uid}"); notify($db, 'order', 'Pro 已取消', "用户#{$uid}"); }
        }
        ...
        if ($act === 'confirm_payment') {
            ...
            notify($db, 'order', '收款开通 Pro', "订单#{$oid} 用户#{$order['user_id']}" . ($expireAt ? " 至{$expireAt}" : ' 终身'));
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .badge-gold { background: linear-gradient(135deg, rgba(250,204,21,0.18), rgba(234,179,8,0.12)); color: #facc15; }
...
<?php $nu = (int)(q1($db, 'SELECT COUNT(*) c FROM qd_notifications WHERE is_read = 0')['c'] ?? 0); ?>
    <a href="?tab=notify" class="<?= $tab==='notify'?'active':'' ?>"><span class="icon">🔔</span> 操作通知<?= $nu ? ' <span class="badge badge-red">'.$nu.'</span>' : '' ?></a>
    <a href="?tab=backup" class="<?= $tab==='backup'?'active':'' ?>"><span class="icon">💾</span> 数据备份</a>
    <a href="?tab=perf" class="<?= $tab==='perf'?'active':'' ?>"><span class="icon">⚡</span> 性能监控</a>
    <a href="?tab=admins" class="<?= $tab==='admins'?'active':'' ?>"><span class="icon">🧑‍💼</span> 管理员</a>
...
INSERT IGNORE INTO qd_admins (username, password) VALUES ('corban', '$2y$10$HASH_PLACEHOLDER');</pre>
  ...
  <?php elseif ($tab === 'notify'): ?>
    <div class="page-title">操作通知</div>
    <div class="page-sub">关键事件实时汇总（开通 / 收款 / 封禁 / 删单 / 管理员变更等）</div>
    <div style="margin-bottom:14px">
      <button class="btn btn-outline" onclick="markAllNotify()">全部标为已读</button>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr><th>类型</th><th>标题</th><th>详情</th><th>时间</th><th>状态</th><th></th></tr></thead>
        <tbody>
        <?php $notes = q($db, 'SELECT * FROM qd_notifications ORDER BY id DESC LIMIT 100'); ?>
        <?php if (empty($notes)): ?><tr><td colspan="6" style="text-align:center;opacity:.3;padding:30px">暂无通知</td></tr>
        <?php else: foreach ($notes as $n): ?>
          <tr style="<?= $n['is_read'] ? '' : 'background:rgba(79,143,255,0.06)' ?>">
            <td><span class="badge badge-purple"><?= e($n['type']) ?></span></td>
            <td><?= e($n['title']) ?></td>
            <td style="opacity:.7;font-size:12px"><?= e($n['detail'] ?? '') ?></td>
            <td style="opacity:.5;font-size:12px"><?= $n['created_at'] ?></td>
            <td><?= $n['is_read'] ? '<span class="badge badge-gray">已读</span>' : '<span class="badge badge-red">未读</span>' ?></td>
            <td><?php if (!$n['is_read']): ?><button class="btn btn-sm" onclick="markNotify(<?= $n['id'] ?>)">标已读</button><?php endif; ?></td>
          </tr>
        <?php endforeach; endif; ?>
        </tbody>
      </table>
  ...
  <?php elseif ($tab === 'backup'): ?>
    ...
    <div class="page-sub">导出全部业务表为 JSON（含用户 / 订阅 / 订单 / 日志 / 通知 / 性能等），便于本地留存与迁移</div>
    <div class="card">
      <div class="card-title">一键导出</div>
      <p style="font-size:13px;opacity:.6;margin-bottom:14px">点击后将下载 <code>quickdial-backup-时间戳.json</code>，内容为所有 qd_* 表的完整数据。</p>
      <a href="?do=backup_export" class="btn btn-primary">⬇ 导出 JSON 备份</a>
    </div>

  <?php elseif ($tab === 'perf'): ?>
    <?php
      $perfTotal = (int)(q1($db, 'SELECT COUNT(*) c FROM qd_perf_log')['c'] ?? 0);
      $perfAvg = (float)(q1($db, 'SELECT AVG(ms) a FROM qd_perf_log')['a'] ?? 0);
      $perfMax = (int)(q1($db, 'SELECT MAX(ms) m FROM qd_perf_log')['m'] ?? 0);
      $perfSlow = q($db, 'SELECT endpoint, COUNT(*) c, AVG(ms) avg_ms, MAX(ms) max_ms FROM qd_perf_log GROUP BY endpoint ORDER BY avg_ms DESC LIMIT 12');
      $perfRecent = q($db, 'SELECT endpoint, ms, created_at FROM qd_perf_log ORDER BY id DESC LIMIT 20');
    ?>
    <div class="page-title">性能监控</div>
    <div class="page-sub">后台接口 / 页面请求耗时（自动采样）</div>
    <div class="stats">
      <div class="stat"><div class="stat-icon">⚡</div><div class="stat-label">采样数</div><div class="stat-value"><?= $perfTotal ?></div></div>
      <div class="stat"><div class="stat-icon">📊</div><div class="stat-label">平均耗时</div><div class="stat-value"><?= round($perfAvg, 1) ?>ms</div></div>
      <div class="stat"><div class="stat-icon">🐢</div><div class="stat-label">最大耗时</div><div class="stat-value" style="color:var(--red)"><?= $perfMax ?>ms</div></div>
    </div>
    <div class="card">
      <div class="card-title">各接口平均耗时（Top 慢）</div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>接口</th><th>次数</th><th>平均</th><th>最大</th></tr></thead>
          <tbody>
          <?php if (empty($perfSlow)): ?><tr><td colspan="4" style="text-align:center;opacity:.3;padding:30px">暂无性能数据</td></tr>
          <?php else: foreach ($perfSlow as $p): ?>
            <tr><td style="font-size:12px"><?= e($p['endpoint']) ?></td><td><?= (int)$p['c'] ?></td><td><?= round((float)$p['avg_ms'], 1) ?>ms</td><td style="color:var(--red)"><?= (int)$p['max_ms'] ?>ms</td></tr>
          <?php endforeach; endif; ?>
          </tbody>
        </table>
      </div>
    ...
    <div class="card">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>接口</th><th>耗时</th><th>时间</th></tr></thead>
          <tbody>
          <?php if (empty($perfRecent)): ?><tr><td colspan="3" style="text-align:center;opacity:.3;padding:30px">暂无数据</td></tr>
          <?php else: foreach ($perfRecent as $p): ?>
            <tr><td style="font-size:12px"><?= e($p['endpoint']) ?></td><td style="<?= (int)$p['ms'] > 500 ? 'color:var(--red)' : '' ?>"><?= (int)$p['ms'] ?>ms</td><td style="opacity:.5;font-size:12px"><?= $p['created_at'] ?></td></tr>
          <?php endforeach; endif; ?>
          </tbody>
        </table>
      </div>
  ...
  <?php elseif ($tab === 'admins'): ?>
    <?php
      $admins = q($db, 'SELECT id, username, role, parent_id, created_at FROM qd_admins ORDER BY id');
      $isSuper = ($_SESSION['qd_admin_role'] ?? 'super') === 'super';
    ?>
    <div class="page-title">管理员</div>
    <div class="page-sub">超管可创建受限子管理员（staff 仅日常运营，不能管理其他管理员）</div>
    <?php if ($isSuper): ?>
    <div class="card">
      <div class="card-title">新增子管理员</div>
      <form onsubmit="addAdmin(event)">
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          <input class="form-input" id="na-user" placeholder="账号" style="flex:1;min-width:140px" />
          <input class="form-input" id="na-pass" type="password" placeholder="密码(≥6位)" style="flex:1;min-width:140px" />
          <select class="form-input" id="na-role" style="flex:1;min-width:120px">
            <option value="staff">受限(staff)</option>
            <option value="manager">运营(manager)</option>
          </select>
    ...
    <div class="card">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>ID</th><th>账号</th><th>角色</th><th>创建时间</th><th></th></tr></thead>
          <tbody>
          <?php foreach ($admins as $a): ?>
            <tr>
              <td><?= $a['id'] ?></td>
              <td><?= e($a['username']) ?><?= $a['id'] == ($_SESSION['qd_admin_id'] ?? 0) ? ' <span class="badge badge-green">我</span>' : '' ?></td>
              <td>
                <?php if ($a['role'] === 'super'): ?><span class="badge badge-gold">超级管理员</span>
                <?php elseif ($a['role'] === 'manager'): ?><span class="badge badge-blue">运营</span>
                <?php else: ?><span class="badge badge-gray">受限</span><?php endif; ?>
              </td>
              <td style="opacity:.5;font-size:12px"><?= $a['created_at'] ?></td>
              <td>
                <?php if ($isSuper && $a['role'] !== 'super'): ?>
                  <button class="btn btn-sm" onclick="setRole(<?= $a['id'] ?>, this)">升为运营</button>
                  <button class="btn btn-sm btn-danger" onclick="delAdmin(<?= $a['id'] ?>)">删除</button>
                <?php endif; ?>
              </td>
            </tr>
          <?php endforeach; ?>
...
function markNotify(id) {
  fetch(location.href, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'mark_notify', id }) })
    .then(() => location.reload());
}
function markAllNotify() {
  fetch(location.href, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'mark_notify' }) })
    .then(() => location.reload());
}
function addAdmin(e) {
  e.preventDefault();
  ...
  if (!username || password.length < 6) { toast('账号或密码(≥6位)无效', 'error'); return; }
  fetch(location.href, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'add_admin', username, password, role }) })
    .then(r => r.json()).then(d => { toast(d.msg || '已添加', d.code === 200 ? 'success' : 'error'); if (d.code === 200) location.reload(); });
}
function setRole(id) {
  fetch(location.href, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'set_admin_role', id, role: 'manager' }) })
    .then(r => r.json()).then(d => { toast(d.msg || '已更新', d.code === 200 ? 'success' : 'error'); if (d.code === 200) location.reload(); });
}
function delAdmin(id) {
  if (!confirm('确定删除该管理员？')) return;
  fetch(location.href, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'del_admin', id }) })
    .then(r => r.json()).then(d => { toast(d.msg || '已删除', d.code === 200 ? 'success' : 'error'); if (d.code === 200) location.reload(); });
}
</script>
<?php endif; ?>
