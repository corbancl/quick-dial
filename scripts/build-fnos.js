...
// 3.5 注入飞牛平台标识（供云同步 detectPlatform 精确区分端，避免 fallback 成 web）
const fnosIndex = path.join(FNOS_UI, 'index.html');
if (fs.existsSync(fnosIndex)) {
  let html = fs.readFileSync(fnosIndex, 'utf-8');
  if (!html.includes('__QD_PLATFORM')) {
    html = html.replace('</head>', `  <script>window.__QD_PLATFORM='fnos';</script>\n</head>`);
    fs.writeFileSync(fnosIndex, html, 'utf-8');
    console.log('🏷️ 已注入 __QD_PLATFORM=fnos');
  }
}
